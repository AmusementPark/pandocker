...
conf-dir="/usr/local/.jenv/candidates/flume/current/conf/"
jenv-init="/usr/local/.jenv/bin/jenv-init.sh"
...
function start_daemon() {
    for agent in ${tier2[@]}
    do
        docker exec -itd flume-${agent} bash -c "source ${jenv-init} && flume-ng agent -n a1 -c ${conf-dir} -f ${agent}.properties &"
    done
